...
RSpec.describe StraightServer::SignatureValidator do
  it 'calculates signature' do
    ...
    expect(described_class.signature(nonce: '12345', body: 'text' * 10_000, method: 'POST', request_uri: '/somewhere', secret: 'gateway_secret')).to eq 'F0GsyqPkxDgmqdTomIGVIRQ/ik2GiZtXy1GVNx0j+UDUL8VS496HsbcOlyUocKUM0fU96KkjhrpUh0LC29AXyQ=='
  end
  ...
  it 'validates nonce in a thread-safe way' do
    ...
    @threads      = thread_number.times.map do |i|
      Thread.new do
        sleep (thread_number - i) / 10_000.0
        Thread.current[:result] = @validator.valid_nonce?
      end
    end
    @threads.each(&:join)
    expect(@threads.count { |thread| thread[:result] }).to eq 1
  end
